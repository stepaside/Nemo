...
namespace Nemo.Extensions
{
    ...
    public static class ObjectExtensions
    {
        ...
        internal static Param[] GetInsertParameters(object businessObject, Dictionary<PropertyInfo, ReflectedProperty> propertyMap, int statementId = -1)
        {
            var parameters = propertyMap.Values
                            .Where(p => p.IsPersistent && (p.CanWrite || p.IsAutoGenerated))
                            .Select(p => new Param
                            {
                                Name = (p.ParameterName.NullIfEmpty() ?? p.PropertyName) + (statementId == -1 ? string.Empty : "_" + statementId),
                                Value = GetParameterValue(businessObject, p.PropertyName),
        ...
        internal static Param[] GetUpdateParameters(object businessObject, Dictionary<PropertyInfo, ReflectedProperty> propertyMap, int statementId = -1)
        {
            var parameters = propertyMap.Values
                    .Where(p => p.IsPersistent && (p.CanWrite || p.IsAutoGenerated))
                    .Select(p => new Param
        ...
        internal static Param[] GetDeleteParameters(object businessObject, Dictionary<PropertyInfo, ReflectedProperty> propertyMap, int statementId = -1)
        {
            var parameters = propertyMap.Values
                    .Where(p => p.CanRead && p.IsPrimaryKey)
                    .Select(p => new Param
        ...
        private static object GetParameterValue(object businessObject, string propertyName)
        {
            var result = Reflector.Property.Get(businessObject.GetType(), businessObject, propertyName);
            if (result != null && Reflector.IsList(result.GetType()))
            {
                var elementType = Reflector.ExtractCollectionElementType(result.GetType());
                if (elementType != null && elementType != typeof(byte) && Reflector.IsSimpleType(elementType))
                {
        ...
        private static void SetOutputParameterValues<T>(T businessObject, IEnumerable<PropertyInfo> outputProperties, Dictionary<PropertyInfo, ReflectedProperty> propertyMap, IList<Param> parameters)
        {
